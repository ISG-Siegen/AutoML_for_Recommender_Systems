...
* Set up a directory X which shall contain all datasets and the preprocessed data
* Create a folder called `preprocessed_data` in this directory. This will be the output directory for preprocessing
* Assuming you have downloaded the original datasets, they need to be inserted in directory X using the following
  approach:
  * **General**: make sure that the relevant data files are in the directory of the dataset. The data file should not be
    in their own directory within the dataset directory.
...
Details on the Docker Setup can be found in `docker_env/README.md`. The following is a shorter version.
...
2. Run the `evaler.py` script with the Python environment from above. This will produce and potentially overwrite
  ...
  * We exported and re-sized the images by hand to save them. If you let the script save the figures, they will look not
    the same.
