...
def load_amazon_toys_from_file():
    data = pd.read_table(os.path.join(get_dataset_default_location(), 'ratings_Toys_and_Games.csv'), sep=',',
    ...
    return 'amazon-toys', data, features, label, recsys_properties


def load_amazon_toys_from_csv():
    data = pd.read_table(os.path.join(get_dataset_default_location(), 'csv_files/amazon-toys.csv'), sep=',',
